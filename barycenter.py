...
class LinearOT:
    ...
    def __init__(self, rho:float=1., reg:float=1e-6):
        self.rho = rho
        self.reg = reg
    ...
    def fit_transform(self, xs, xt, ws, wt):
        self.As, self.bs, self.At, self.bt = self._fit(xs, xt, ws, wt)
...
def compute_transformation(cs, ct, ms, mt, rho=1., reg=1e-6):
	"""
...
def barycenter(xs, xt, rho=1., ws=None, wt=None, reg:float=1e-6, ):
	"""
